...
run_num_and_dump()
{
	...
	do
		...
		then
			printf "🔥 "
			printf "\nCommands to run:" >> $dir/diff/diff_log_$i.txt
			printf "./VM_tests/corewar ${num_and_dump_test[$i]}" >> $dir/diff/diff_log_$i.txt
			printf "./VM_tests/corzaz ${num_and_dump_test[$i]}" >> $dir/diff/diff_log_$i.txt
			printf "\n"
			let "failed++"
		else
			printf "$COLOR.$END"
...
run_battle()
{
	...
	do
		...
		then
			printf "🔥 "
			printf "\nCommands to run:" >> $dir/diff/diff_log_$i.txt
			printf "./VM_tests/corewar ${battle_test[$i]}" >> $dir/diff/diff_log_$i.txt
			printf "./VM_tests/corzaz ${battle_test[$i]}" >> $dir/diff/diff_log_$i.txt
			printf "\n"
			let "failed++"
		else
			printf "$COLOR.$END"
...
# 	    ./$exec ${valid_test[$i]} > $VM_dir/valid_champions/natapol/natapol_output_$i 2>&1
